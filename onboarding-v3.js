// IntegraTrampo · onboarding profissional v3
// Cadastro com conta autenticada, foto obrigatória (câmera ou galeria) e moderação humana.
(function(){
  ...
  function injectStyles(){
    ...
    const s=document.createElement('style');s.id='itOnboardingV3Styles';s.textContent=`
      ...
      @media(max-width:560px){.work-chip-grid{grid-template-columns:1fr 1fr}.work-chip{padding:9px;font-size:13px}.schedule-row{grid-template-columns:1fr 78px 14px 78px;padding:8px}.schedule-row input[type=time]{font-size:12px;padding:7px 3px}.photo-actions .btn{flex:1}}
  ...
  function bindControls(){
    ...
    document.querySelectorAll('[data-schedule-day]').forEach(row=>{const check=row.querySelector('.schedule-enabled'),start=row.querySelector('.schedule-start'),end=row.querySelector('.schedule-end');check?.addEventListener('change',()=>{const on=check.checked;if(start)start.disabled=!on;if(end)end.disabled=!on})});
    const setDays=codes=>document.querySelectorAll('[data-schedule-day]').forEach(row=>{const on=codes.includes(row.dataset.scheduleDay),check=row.querySelector('.schedule-enabled'),start=row.querySelector('.schedule-start'),end=row.querySelector('.schedule-end');if(check)check.checked=on;if(start)start.disabled=!on;if(end)end.disabled=!on});
  ...
  function readSchedule(){
    ...
    document.querySelectorAll('[data-schedule-day]').forEach(row=>{if(!row.querySelector('.schedule-enabled')?.checked)return;const code=row.dataset.scheduleDay,label=row.dataset.dayLabel||code,start=row.querySelector('.schedule-start')?.value||'08:00',end=row.querySelector('.schedule-end')?.value||'18:00';schedule[code]={label,start,end};const short=(WEEK.find(x=>x[0]===code)||[])[2]||label;summary.push(`${short} ${start}–${end}${end<start?' (+1 dia)':''}`)});
    return {schedule,summary:summary.join(' · ')};
  }

  async function imageInfo(file){
    return await new Promise((resolve,reject)=>{const u=URL.createObjectURL(file),img=new Image();img.onload=()=>{const out={width:img.naturalWidth,height:img.naturalHeight,url:u,img};resolve(out)};img.onerror=()=>{URL.revokeObjectURL(u);reject(new Error('invalid_image'))};img.src=u});
  }
  ...
  async function preparePhoto(file){
    ...
    try{
      ...
      if(previewUrl)URL.revokeObjectURL(previewUrl);previewUrl=URL.createObjectURL(photoFile);const img=$id('wPhotoPreview');if(img){img.src=previewUrl;img.style.display='block'}
      if(status)status.textContent='✅ Foto pronta para envio e análise';
    }catch(e){photoFile=null;const msg=e.message==='too_small'?'Use uma foto com pelo menos 320×320 px.':e.message==='too_large'?'A imagem original é grande demais. Escolha outra foto.':e.message==='invalid_type'?'Use JPG, PNG ou WebP.':'Não foi possível usar essa imagem.';if(status)status.textContent='⚠️ '+msg;tell(msg)}
  }

  function requireLoginForWorker(){
    try{localStorage.setItem(PENDING_KEY,'worker')}catch{}
    if(typeof loginModal==='function'){loginModal();setTimeout(()=>{const box=document.querySelector('#modalRoot .notice');if(box)box.innerHTML='🔐 <b>Primeiro confirme seu e-mail.</b> Depois do link seguro você volta para concluir o cadastro, incluindo a foto obrigatória.'},0)}
  ...
  window.workerModal=async function(){
    ...
    modal(`<div class="notice">🛡️ <b>Cadastro com revisão humana.</b> Sua foto é obrigatória, mas só fica pública depois que a IntegraTrampo aprovar o perfil.</div>
      <h2>Quero trabalhar</h2><div class="form-grid">
      <div class="field"><label>Nome completo</label><input id="wName" placeholder="Seu nome"></div><div class="field"><label>WhatsApp</label><input id="wPhone" inputmode="tel" placeholder="(18) 99999-9999"></div>
      <div class="field full"><label>E-mail da conta</label><input id="wEmail" type="email" value="${esc(user.email||'')}" readonly></div>
      <div class="field full"><label>Foto de perfil obrigatória</label><div class="photo-onboard"><img id="wPhotoPreview" class="photo-preview" alt="Prévia da sua foto"><div class="photo-actions"><label class="btn btn--navy" for="wPhotoCamera">📷 Tirar foto agora</label><label class="btn btn--outline" for="wPhotoGallery">🖼️ Escolher da galeria</label></div><input id="wPhotoCamera" type="file" accept="image/*" capture="user" style="display:none"><input id="wPhotoGallery" type="file" accept="image/jpeg,image/png,image/webp" style="display:none"><div id="wPhotoStatus" class="photo-status">Nenhuma foto selecionada</div><div class="photo-checklist">Para facilitar a aprovação: apareça sozinho, de frente, com o rosto visível, boa iluminação e sem foto de outra foto/tela. A imagem será comprimida no celular antes do envio quando necessário.</div></div></div>
  ...
  window.saveWorker=async function(){
    ...
    if(!isEmail(email)){tell('E-mail da conta inválido');return}
    ...
    }catch(e){console.error('worker signup v3',e);const m=String(e?.message||'');tell(m.includes('photo_required')?'A foto é obrigatória para concluir o cadastro.':'Não foi possível concluir o cadastro agora');if(btn){btn.disabled=false;btn.textContent='Enviar cadastro para análise'}}
  };

  async function resumePending(user){
    if(!user)return;let pending='';try{pending=localStorage.getItem(PENDING_KEY)||''}catch{};if(pending==='worker'){try{localStorage.removeItem(PENDING_KEY)}catch{};setTimeout(()=>window.workerModal(),650)}
  }
