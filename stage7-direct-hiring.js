...
(function(){
  ...
  const fmtRange=(start,end)=>start&&end?`${fmtTime(start)}–${fmtTime(end)}${String(end)<String(start)?' (+1 dia)':''}`:'';
  ...
  function offerCard(o){
    ...
    const schedule=[fmtDate(o.service_date),o.start_time&&o.end_time?fmtRange(o.start_time,o.end_time):null].filter(Boolean).join(' · ');
  ...
  window.IntegraTrampoStage7DirectHiring={version:2,get offers(){return offers},refresh:renderOffers};
})();